...
export default class MenuView extends View {
	...
	*/
	...
	createDebugSVG(x, y, w, h, txt, fontsize) {
		const svgNS = 'http://www.w3.org/2000/svg';
...
<image x="610" y="280" width="149.88" height="100" xlink:href="640px-Flag_of_Europe.svg.png" />
	*/
	...
	appendDEBUGInfo() {
		...
		const x_pos = -w*0.5+12;
		const y_pos = h*0.5-12;
		
		const group = document.createElementNS(svgNS, "g");
		
		const debug_text = "userid="+this.USER_MODEL.id+" country=" +this.USER_MODEL.profile['Country']+" language="+
			this.LM.selected+" MOCKUP="+this.controller.master.MOCKUP;
			
		let fontsize = 12;
		// If there is room to put all text to one line => do it.
		// Create ONE SVG and put a rect and text under it.
		group.appendChild(this.createDebugSVG(
			x_pos,
			y_pos,
			w - 24,
			12,
	...
	notify(options) {
		if (this.controller.visible) {
			if (options.model==='ResizeEventObserver' && options.method==='resize') {
				...
				console.log('User MODEL is now RESTORED!!!!!');
				this.show();
			}
		}
	}
