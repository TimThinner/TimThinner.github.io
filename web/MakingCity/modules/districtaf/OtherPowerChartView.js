...
export default class OtherPowerChartView extends View {
			...
			if ((options.model==='Other109Model'||options.model==='Other111Model') && options.method==='fetched') {
				if (this.rendered) {
					if (options.status === 200) {
						...
						$('#other-power-chart-view-failure').empty();
						
						if (typeof this.chart !== 'undefined') {
							console.log('fetched ..... OtherPowerChartView CHART UPDATED!');
							//109								Car heating
							//111								VSS lighting
							am4core.iter.each(this.chart.series.iterator(), function (s) {
								if (s.name === 'Car heating') {
									s.data = self.models['Other109Model'].values;
								} else {
									s.data = self.models['Other111Model'].values;
								}
							});
							
						} else {
							console.log('fetched ..... OtherPowerChartView renderChart()');
							this.renderChart();
						}
					} else { // Error in fetching.
						$('#other-power-chart-view-failure').empty();
						const html = '<div class="error-message"><p>'+options.message+'</p></div>';
						$(html).appendTo('#other-power-chart-view-failure');
					}
				}
			}
		}
	...
	renderChart() {
		...
		am4core.ready(function() {
			...
			// Create chart
			self.chart = am4core.create("other-power-chart", am4charts.XYChart);
			...
			*/
			...
			series1.data = self.models['Other109Model'].values;
			...
			series2.data = self.models['Other111Model'].values;
			...
			*/
			...
			console.log('OTHER POWER RENDER CHART END =====================');
	...
	render() {
		...
		const html =
			...
			'</div>'+
			'<div class="row">'+
				'<div class="col s12" id="other-power-chart-view-failure"></div>'+
			'</div>';
		...
		if (this.areModelsReady()) {
			...
			if (errorMessages.length > 0) {
				const html =
					'<div class="row">'+
						'<div class="col s12 center" id="other-power-chart-view-failure">'+
