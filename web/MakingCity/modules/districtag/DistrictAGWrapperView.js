...
import CoolerPowerChartView from './CoolerPowerChartView.js';
import CoolerEnergyChartView from './CoolerEnergyChartView.js';

export default class DistrictAGWrapperView extends WrapperView {
	
	constructor(controller) {
		super(controller);
		
		// This is a wrapper for two different "subviews":
		//   - Chart which contains Cooler POWER for current day
		//   - Chart which contains Cooler ENERGY for current day
		
		this.subviews.push(new CoolerPowerChartView(this, '#subview-1'));
		this.subviews.push(new CoolerEnergyChartView(this, '#subview-2'));
	}
	...
	render() {
		...
		const localized_string_dag_title = LM['translation'][sel]['DAG_TITLE'];
		...
		if (um.isLoggedIn()) {
			...
			this.controller.models['Cooler113Model'].timerange = 1;
			this.controller.models['Cooler112Model'].timerange = 1;
		}
		
		const html = 
			'<div class="row">'+
				'<div class="col s12 center">'+
					'<h3 class="da-wrapper-title">'+localized_string_dag_title+'</h3>'+
				'</div>'+
			'</div>'+ extra +
			'<div class="row">'+
		...
		this.setTimerangeHandlers(['Cooler113Model','Cooler112Model']);
