...
export default class GeothermalPowerChartView extends View {
	...
	constructor(wrapper, el) {
		...
		this.timerName = 'GeothermalChartView';
			...
			if (options.model==='GeothermalModel' && options.method==='fetched') {
				if (this.rendered) {
					if (options.status === 200) {
						...
						$(html).appendTo('#geothermal-power-chart-view-failure');
					}
				}
			}
		}
	...
	render() {
		...
		const html =
			...
			'</div>'+
			'<div class="row">'+
				'<div class="col s12" id="geothermal-power-chart-view-failure"></div>'+
			'</div>';
		...
		if (this.areModelsReady()) {
			...
			if (errorMessages.length > 0) {
				const html =
					'<div class="row">'+
						'<div class="col s12 center" id="geothermal-power-chart-view-failure">'+
