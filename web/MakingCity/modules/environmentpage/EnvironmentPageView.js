		...
		if (this.controller.visible) {
			if (options.model==='EnvironmentPageModel' && options.method==='fetched') {
				if (options.status === 200) {
					...
					if (this.rendered) {
						$('#environment-page-view-failure').empty();
					...
					if (this.rendered) {
						...
						if (options.status === 401) {
							...
							$(html).appendTo('#environment-page-view-failure');
							setTimeout(() => {
				...
				console.log("EnvironmentPageView ResizeEventObserver resize!!!!!!!!!!!!!!");
				this.render();
			}*/
		}
	}
	...
	render() {
		...
		if (this.areModelsReady()) {
			...
			if (errorMessages.length > 0) {
				const html =
					'<div class="row">'+
						'<div class="col s12 center" id="environment-page-view-failure">'+
				...
				if (errorMessages.indexOf('Auth failed') >= 0) {
					...
					$('<div class="error-message"><p>Session has expired... logging out in 3 seconds!</p></div>').appendTo('#environment-page-view-failure');
					setTimeout(() => {
						this.controller.forceLogout();
					}, 3000);
				}
				...
				const html =
					...
					'</div>'+
					'<div class="row">'+
						'<div class="col s12 center" id="environment-page-view-failure"></div>'+
					'</div>';
					*/
