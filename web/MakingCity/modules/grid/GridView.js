...
export default class GridView extends View {
	...
	notify(options) {
		if (this.controller.visible) {
			if (options.model==='GridModel' && options.method==='fetched') {
				if (options.status === 200) {
					console.log('GridView => GridModel fetched!');
					if (this.rendered) {
						$('#grid-view-failure').empty();
					...
					if (this.rendered) {
						...
						if (options.status === 401) {
							...
							$(html).appendTo('#grid-view-failure');
							setTimeout(() => {
				...
				console.log("GridView ResizeEventObserver resize!!!!!!!!!!!!!!");
				this.render();
			}*/
		}
	}
	...
	render() {
		...
		if (this.areModelsReady()) {
			...
			if (errorMessages.length > 0) {
				const html =
					'<div class="row">'+
						'<div class="col s12 center" id="grid-view-failure">'+
				...
				if (errorMessages.indexOf('Auth failed') >= 0) {
					...
					$('<div class="error-message"><p>Session has expired... logging out in 3 seconds!</p></div>').appendTo('#grid-view-failure');
					setTimeout(() => {
						this.controller.forceLogout();
					}, 3000);
				}
				...
				const localized_string_title = LM['translation'][sel]['GRID_TITLE'];
				const localized_string_description = LM['translation'][sel]['GRID_DESCRIPTION'];
				...
				const html =
					...
					'</div>'+
					'<div class="row">'+
						'<div class="col s12 center" id="grid-view-failure"></div>'+
					'</div>';
					*/
			...
			console.log('GridView => render Model IS NOT READY!!!!');
			// this.el = '#content'
