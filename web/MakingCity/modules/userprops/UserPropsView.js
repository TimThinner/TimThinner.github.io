	...
	notify(options) {
		if (this.controller.visible) {
			if (options.model==='UserPropsModel' && options.method==='fetched') {
				if (options.status === 200) {
					...
					if (this.rendered) {
						...
						if (options.status === 401) {
							...
							$(html).appendTo('#user-props-view-failure');
							setTimeout(() => {
				...
				console.log("UserPropsView ResizeEventObserver resize!!!!!!!!!!!!!!");
				this.render();
			}*/
		}
	}
		...
		if (this.areModelsReady()) {
			...
			if (errorMessages.length > 0) {
				...
				if (errorMessages.indexOf('Auth failed') >= 0) {
					...
					$('<div class="error-message"><p>Session has expired... logging out in 3 seconds!</p></div>').appendTo('#user-props-view-failure');
					setTimeout(() => {
						this.controller.forceLogout();
					}, 3000);
				}
