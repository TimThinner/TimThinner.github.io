...
/*
		...
		this.models = {};
*/
export default class ReadKeyEditView extends View {
	...
	notify(options) {
		if (this.controller.visible) {
			if (options.model==='ReadKeyModel' && options.method==='updateOne') {
				...
				const context = this.models['ReadKeyModel'].getContext();
	...
	render() {
		...
		*/
		...
		$('#update-readkey').on('click',function() {
			...
			*/
			...
			if (date_errors.length > 0) {
				...
				self.models['ReadKeyModel'].updateOne(sid, data, authToken);
			}
		});
	}
}
