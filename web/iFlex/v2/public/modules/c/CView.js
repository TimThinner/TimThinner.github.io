...
export default class CView extends TimeRangeView {
	...
	*/
	findClosestFactor(key) {
		...
		this.models['BuildingEmissionFactorForElectricityConsumedInFinlandModel'].values.every(v=>{
			const val = +v.value; // convert value to number
			const m = moment(v.timestamp);
			const dist = Math.abs(m.unix() - emm.unix()); // moment#unix outputs a Unix timestamp (the number of seconds since the Unix Epoch).
			if (dist < distance) {
				distance = dist;
				retval = val;
			}
			if (distance === 0) { // You cannot get closer than this!
				return false; // so break out from the loop.
			}
			// Make sure you return true. If you don't return a value, `every()` will stop.
			return true;
		});
		//console.log(['findClosestFactor FACTOR=',retval,' DISTANCE=',distance,' key=',key]);
		return retval;
	}
	...
	*/
	...
	calculate_DH_Sum() {
		...
		this.models['CControllerBuildingHeatingQE01Model'].values.forEach(v=>{
			...
			const val = v.value * 182/1000; // Converts string to number.
	...
	calculateSum() {
		...
		console.log(['calculateSum FACTORS LEN=',this.models['BuildingEmissionFactorForElectricityConsumedInFinlandModel'].values.length]);
		...
		//console.log(['Performance: ',dms,'ms']); // 36-40 ms
	...
	*/
	...
	calculate_ALL() {
		...
		if (this.models['CControllerBuildingElectricityPL1Model'].values.length > 0 && 
			...
			this.calculateSum(); // Sum 3 ele PL1, PL2, PL3 and put this.calculated_EL_emissions
	...
	renderChartComparison() {
		...
		am4core.ready(function() {
			...
			*/
			//var series = self.chart_comparison.series.push(new am4charts.ColumnSeries());
			var series = self.chart_comparison.series.push(new am4charts.LineSeries());
			series.data = self.calculated_USER_emissions; 
			series.dataFields.dateX = "timestamp";
			series.dataFields.valueY = "value";
			series.tooltipText = localized_string_emission_user + ": [bold]{valueY.formatNumber('#.#')}[/] gCO2/h";
			series.fillOpacity = 0.25;
			series.name = 'USEREMISSIONS';
			series.customname = localized_string_emission_user_legend;
			...
			*/
			...
			self.chart_comparison.scrollbarX.series.push(series);
